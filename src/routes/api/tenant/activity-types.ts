...
const PUT = withAuth(PERMISSIONS.SETTINGS_MANAGE, async ({ request, auth }) => {
  ...
  const data = await setTenantActivityTypes(auth.userId, body.activityTypeCodes)
  return Response.json({ success: true, data })
})

export const Route = createFileRoute('/api/tenant/activity-types')({
  server: {
    handlers: {
      GET,
      PUT,
