...
import { createLookupType, listLookupTypes, type CreateLookupTypeInput } from '@/server/fns/lookups'
...
const POST = withAuth(PERMISSIONS.INVENTORY_MANAGE, async ({ request, auth }) => {
  try {
    const { userId } = auth
    const body = (await request.json()) as CreateLookupTypeInput
    const data = await createLookupType(userId, body)
    return Response.json({ success: true, data })
  } catch (error) {
    return handleRouteError(error, 'Unable to create lookup catalog')
  }
})
