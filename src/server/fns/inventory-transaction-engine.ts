...
export async function createInventoryTransaction(
  ...
  return runWithTenantContext({ tenantId, userId: authUserId }, async () => {
    ...
    if (!Array.isArray(input.items) || input.items.length === 0) {
      throw new ApiError('Inventory transaction must contain at least one item.', 400)
    }
    ...
    for (const item of input.items) {
      ...
      if (qty.lte(0)) {
        throw new ApiError('Item quantity must be strictly greater than 0.', 400)
      }
    }

    // 3. Resolve Transaction Type
    const txnType = await prisma.inventory_transaction_types.findUnique({
      where: { code: input.typeCode },
      include: { rules: true },
    })

    if (!txnType || !txnType.is_active) {
      throw new ApiError(`Transaction type '${input.typeCode}' does not exist or is inactive.`, 404)
    }

    if (txnType.requires_destination && !input.destWarehouseId && !input.destStoreId) {
      const missingDest = input.items.some((i) => !i.destWarehouseId && !i.destStoreId)
      if (missingDest) {
        throw new ApiError(`Transaction type '${input.typeCode}' requires a destination warehouse or store.`, 400)
      }
    }
    ...
    const preparedItems = input.items.map((item, index) => {
      ...
      return {
        ...
        source_warehouse_id: item.sourceWarehouseId ?? input.sourceWarehouseId ?? null,
        source_store_id: item.sourceStoreId ?? input.sourceStoreId ?? null,
        source_location_id: item.sourceLocationId ?? input.sourceLocationId ?? null,
        dest_warehouse_id: item.destWarehouseId ?? input.destWarehouseId ?? null,
    ...
    const createdTxn = await prisma.inventory_transactions.create({
      data: {
        tenant_id: tenantId,
    ...
    if (input.autoPost || txnType.auto_post) {
      return await postInventoryTransaction(authUserId, createdTxn.id)
    }
  ...
  return runWithTenantContext({ tenantId, userId: authUserId }, async () => {
    return await prisma.$transaction(async (tx) => {
      ...
      if (txn.status !== 'draft' && txn.status !== 'pending') {
        throw new ApiError(`Cannot post transaction in '${txn.status}' status.`, 400)
      }
      ...
      for (const item of txn.items) {
        ...
        const sourceRules = rules.filter((r) => r.applies_to === 'SOURCE' || r.applies_to === 'BOTH')
        if (sourceRules.length > 0) {
          ...
          let available = toDecimal(sourceBalance.qty_available ?? sourceBalance.qty_on_hand)
          ...
          for (const rule of sourceRules) {
            ...
            if (rule.stock_field === 'ON_HAND') {
              onHand = op === 'ADD' ? onHand.plus(delta) : op === 'SUBTRACT' ? onHand.minus(delta) : onHand
            } else if (rule.stock_field === 'AVAILABLE') {
              available = op === 'ADD' ? available.plus(delta) : op === 'SUBTRACT' ? available.minus(delta) : available
            } else if (rule.stock_field === 'RESERVED') {
              reserved = op === 'ADD' ? reserved.plus(delta) : op === 'SUBTRACT' ? reserved.minus(delta) : reserved
            } else if (rule.stock_field === 'IN_TRANSIT') {
              inTransit = op === 'ADD' ? inTransit.plus(delta) : op === 'SUBTRACT' ? inTransit.minus(delta) : inTransit
            } else if (rule.stock_field === 'DAMAGED') {
              damaged = op === 'ADD' ? damaged.plus(delta) : op === 'SUBTRACT' ? damaged.minus(delta) : damaged
            }
          }
        ...
        const destRules = rules.filter((r) => r.applies_to === 'DESTINATION' || r.applies_to === 'BOTH')
        if (destRules.length > 0) {
          ...
          let available = toDecimal(destBalance.qty_available ?? destBalance.qty_on_hand)
          ...
          if (unitCost.gt(0) && (txn.transaction_type.category === 'purchase' || txn.transaction_type.category === 'opening')) {
        ...
        const legacyMovementType = mapCategoryToLegacyMovement(txn.transaction_type.category, txn.direction)
        const primaryWarehouseId = item.dest_warehouse_id ?? txn.dest_warehouse_id ?? item.source_warehouse_id ?? txn.source_warehouse_id
        const primaryStoreId = item.dest_store_id ?? txn.dest_store_id ?? item.source_store_id ?? txn.source_store_id
        const primaryLocationId = item.dest_location_id ?? txn.dest_location_id ?? item.source_location_id ?? txn.source_location_id
        ...
        await tx.inventory_movements.create({
          data: {
            ...
            dest_warehouse_location_id: item.dest_location_id ?? txn.dest_location_id,
            batch_id: item.batch_id,
            serial_id: item.serial_id,
...
export async function cancelInventoryTransaction(
  ...
  return runWithTenantContext({ tenantId, userId: authUserId }, async () => {
    ...
    if (txn.status === 'posted') {
      throw new ApiError('A posted inventory transaction cannot be cancelled. Use reverseTransaction() instead.', 400)
    }
    ...
    const cancelled = await prisma.inventory_transactions.update({
      ...
      data: {
        ...
        notes: reason ? `${txn.notes ?? ''}\nCancellation reason: ${reason}`.trim() : txn.notes,
        updated_by_user_id: tenantUserId,
      },
    })
...
export async function reverseInventoryTransaction(
  ...
  return runWithTenantContext({ tenantId, userId: authUserId }, async () => {
    ...
    if (original.status !== 'posted') {
      throw new ApiError(`Cannot reverse transaction in '${original.status}' status. Only posted transactions can be reversed.`, 400)
    }
    ...
    const reversalItems: InventoryTransactionItemInput[] = original.items.map((item) => ({
      productVariantId: item.product_variant_id,
      quantity: item.quantity,
      unitCost: item.unit_cost,
      sourceWarehouseId: original.direction === 'inbound' ? item.dest_warehouse_id : item.source_warehouse_id,
      sourceStoreId: original.direction === 'inbound' ? item.dest_store_id : item.source_store_id,
      destWarehouseId: original.direction === 'outbound' ? item.source_warehouse_id : item.dest_warehouse_id,
      destStoreId: original.direction === 'outbound' ? item.source_store_id : item.dest_store_id,
    ...
    await prisma.inventory_audit_logs.create({
      data: {
        ...
        new_values: { status: 'reversed', reversed_by_transaction_id: reversalTxn.id },
        user_id: tenantUserId,
        metadata: { reason, reversal_transaction_number: reversalTxn.transaction_number },
      },
    })
...
export async function reserveInventoryStock(
  ...
  return runWithTenantContext({ tenantId, userId: authUserId }, async () => {
    ...
    const available = toDecimal(balance?.qty_available ?? balance?.qty_on_hand ?? 0)
...
export async function releaseInventoryReservation(
  ...
  return runWithTenantContext({ tenantId, userId: authUserId }, async () => {
    ...
    if (reservation.status !== 'active') {
      throw new ApiError(`Reservation is in '${reservation.status}' status, cannot be released.`, 400)
    }
...
function mapCategoryToLegacyMovement(category: string, direction: string): string {
  switch (category) {
    case 'purchase':
      return 'purchase'
    case 'sale':
    ...
    case 'production':
      return direction === 'outbound' ? 'production_consumption' : 'production_output'
    case 'opening':
