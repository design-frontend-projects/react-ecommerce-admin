    ...
    return {
      data: rows.map((r) => ({
        ...
        rules: r.rules?.map((rule) => ({
          ...rule,
          discount_value: Number(rule.discount_value?.toString() ?? 0),
          discountValue: Number(rule.discount_value?.toString() ?? 0),
          tier_min_quantity: rule.tier_min_quantity ? Number(rule.tier_min_quantity.toString()) : null,
          tier_min_amount: rule.tier_min_amount ? Number(rule.tier_min_amount.toString()) : null,
          get_discount_percent: rule.get_discount_percent ? Number(rule.get_discount_percent.toString()) : 100,
        })),
      })),
      totalCount,
      page,
