...
import { Printer, X, Building2, User, Phone, Mail, Calendar, CreditCard, ShieldCheck } from 'lucide-react'
        ...
        <div className="overflow-y-auto p-8 bg-white text-slate-900 font-sans print:p-0 print:overflow-visible">
          ...
          <div className="flex justify-between items-start border-b border-slate-200 pb-6">
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500 font-medium">Commercial Invoicing & Financial Operations</p>
              <div className="text-xs text-slate-600 space-y-0.5 pt-2">
                <div>Tax ID: TR-89201948291</div>
                <div>support@enterprise-erp.local | +1 (800) 555-0199</div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-6 py-6 border-b border-slate-200 text-xs">
            ...
            <div className="space-y-1">
              ...
              <div className="pt-1 text-slate-500">
                Source Channel: <strong className="text-slate-700">{invoice.source_type} ({invoice.channels?.name || 'Counter'})</strong>
              </div>
            </div>
            ...
            <div className="space-y-2 text-right">
              <div className="space-y-0.5">
                ...
                <div className="text-slate-700">
                  Date: <strong className="text-slate-900">{new Date(invoice.invoice_date).toLocaleDateString()}</strong>
                </div>
                {invoice.due_date && (
                  <div className="text-slate-700">
                    Due Date: <strong className="text-slate-900">{new Date(invoice.due_date).toLocaleDateString()}</strong>
                  </div>
                )}
                {invoice.warehouses && (
                  <div className="text-slate-600">
                    Fulfillment: <strong>{invoice.warehouses.name}</strong>
                  </div>
                )}
              </div>
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b-2 border-slate-300 text-slate-500 font-semibold uppercase tracking-wider text-[10px]">
                  ...
                  <th className="py-2.5 text-right w-20">Tax</th>
                ...
                {invoice.sales_invoice_payments.map((p) => (
                  ...
                  >
                    <div className="flex items-center gap-2 text-slate-700">
                      ...
                      <span className="capitalize font-medium">{p.payment_method.replace('_', ' ')}</span>
                      {p.reference_number && (
          ...
          <div className="mt-10 pt-6 border-t border-slate-200 flex justify-between items-end text-slate-500 text-[11px]">
            <div className="space-y-1 max-w-md">
              ...
              <p>{invoice.terms || 'Payment is due according to agreed billing credit terms. All returns subject to store policy.'}</p>
              {invoice.notes && <p className="text-slate-600 italic">Note: {invoice.notes}</p>}
            </div>
            <div className="text-right space-y-10">
              <div className="w-40 border-b border-slate-300"></div>
              <div className="text-slate-400 text-[10px]">Authorized Commercial Signatory</div>
            </div>
          </div>
        </div>
      </DialogContent>
