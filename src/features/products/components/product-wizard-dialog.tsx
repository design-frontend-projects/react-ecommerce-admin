...
export function ProductWizardDialog() {
  ...
  const handleCreate = async (variantsData: VariantRowFormData[]) => {
    ...
    try {
      ...
      toast.error(
        `Failed to create product: ${(error as Error).message || 'Unknown error'}`
      )
    }
  }
      ...
      <DialogContent className='sm:max-w-[800px]'>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            {currentStep === 1 ? 'Base Information' : 'Product Variants'}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className='my-2 flex items-center justify-center space-x-2 text-sm'>
          ...
          >
            <div
              className={`flex h-6 w-6 items-center justify-center rounded-full border ${currentStep >= 1 ? 'border-primary bg-primary text-primary-foreground' : ''}`}
            >
              1
            </div>
          ...
          >
            <div
              className={`flex h-6 w-6 items-center justify-center rounded-full border ${currentStep >= 2 ? 'border-primary bg-primary text-primary-foreground' : ''}`}
            >
