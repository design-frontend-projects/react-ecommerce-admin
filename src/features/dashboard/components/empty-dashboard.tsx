  ...
  return (
    <div className='flex flex-col items-center justify-center min-h-[60vh] p-6 text-center animate-in fade-in-50 zoom-in-95 duration-500'>
      ...
      <h2 className='text-2xl md:text-3xl font-bold tracking-tight text-foreground mb-2'>
        {t('dashboard.emptyState.welcome', 'Welcome to your Inventory Command Center')}
      </h2>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-2xl w-full mb-8'>
        <Card className='border-border/60 hover:border-primary/50 transition-all hover:shadow-md bg-card/60 backdrop-blur-xs'>
          <CardContent className='p-5 flex flex-col items-center text-center'>
            ...
            <h4 className='font-semibold text-sm mb-1 text-foreground'>
              {t('dashboard.emptyState.step1Title', '1. Add Products')}
            </h4>
            <p className='text-xs text-muted-foreground mb-4'>
              {t(
                'dashboard.emptyState.step1Description',
                'Define SKUs, categories, and inventory minimum levels.'
              )}
            </p>
            <Button variant='outline' size='sm' asChild className='w-full mt-auto text-xs'>
              <Link to='/products'>
                {t('dashboard.emptyState.step1Action', 'Create Product')}{' '}
        ...
        <Card className='border-border/60 hover:border-primary/50 transition-all hover:shadow-md bg-card/60 backdrop-blur-xs'>
          <CardContent className='p-5 flex flex-col items-center text-center'>
            ...
            <h4 className='font-semibold text-sm mb-1 text-foreground'>
              {t('dashboard.emptyState.step2Title', '2. Purchase Orders')}
            </h4>
            <p className='text-xs text-muted-foreground mb-4'>
              {t(
                'dashboard.emptyState.step2Description',
                'Order inventory from suppliers to populate your stock.'
              )}
            </p>
            <Button variant='outline' size='sm' asChild className='w-full mt-auto text-xs'>
              <Link to='/purchase-orders'>
                {t('dashboard.emptyState.step2Action', 'New Purchase Order')}{' '}
                <ArrowRight className='w-3 h-3 ml-1' />
              </Link>
            </Button>
          </CardContent>
        ...
        <Card className='border-border/60 hover:border-primary/50 transition-all hover:shadow-md bg-card/60 backdrop-blur-xs'>
          <CardContent className='p-5 flex flex-col items-center text-center'>
            ...
            <h4 className='font-semibold text-sm mb-1 text-foreground'>
              {t('dashboard.emptyState.step3Title', '3. Initial Stock')}
            </h4>
            <p className='text-xs text-muted-foreground mb-4'>
              {t(
                'dashboard.emptyState.step3Description',
                'Directly import or adjust opening physical inventory.'
              )}
            </p>
            <Button variant='outline' size='sm' asChild className='w-full mt-auto text-xs'>
              <Link to='/inventory-movements'>
                {t('dashboard.emptyState.step3Action', 'Adjust Stock')}{' '}
                <ArrowRight className='w-3 h-3 ml-1' />
              </Link>
            </Button>
          </CardContent>
      ...
      {onRefresh && (
        ...
        >
          ...
          {isRefetching
            ? t('dashboard.emptyState.checkingUpdates', 'Checking for updates...')
            : t('dashboard.emptyState.recheckDatabase', 'Re-check database')}
        </Button>
      )}
    </div>
  )
