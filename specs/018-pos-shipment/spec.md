...
3. **Given** valid shipment details are provided, **When** I process the payment, **Then** the order should be saved and a corresponding shipment record should be created with the current `auth_user_id`.
...
- **FR-004**: System MUST automatically associate the `auth_user_id` of the logged-in employee with the shipment record.
...
- **res_shipments**:
    ...
    - `auth_user_id` (String) - The employee who created the shipment (Required)
...
- **SC-002**: 100% of shipment orders have a corresponding record in the `res_shipments` table with valid `auth_user_id`.
...
- Authentication (Clerk) is already configured and `auth_user_id` is available in the current context.
